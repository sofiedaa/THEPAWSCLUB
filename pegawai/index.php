...
<tbody>
  ...
  <tr>
    <td><?= $i++ ?></td>
    <td><?= $row['ID_PEGAWAI'] ?></td> <!-- ini juga tambahan -->
    <td><?= $row['NAMA'] ?></td>
    <td><?= $row['POSISI'] ?></td>
    <td><?= $row['NO_TELP'] ?></td>
    <td><?= $row['EMAIL'] ?></td>
    <td>
      <a href="edit.php?id=<?= $row['ID_PEGAWAI'] ?>" class="btn btn-warning btn-sm">Edit</a>
      <a href="function.php?Aksi=Hapus&id=<?= $row['ID_PEGAWAI'] ?>" 
